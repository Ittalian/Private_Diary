...
class TestDiaryCreateView(LoggedInTestCase):
    def test_cretae_diary_success(self):
        #POSTパラメータ
...
class TestDiaryUpdateView(LoggedInTestCase):
    def test_update_diary_success(self):
        ...
        response = self.client.post(reverse_lazy('diary_update', kwargs = {'pk':diary.pk}), params)

        self.assertRedirects(response, reverse_lazy('diary_detail', kwargs = {'pk':diary.pk}))
        self.assertEqual(Diary.objects.get(pk = diary.pk).title, 'タイトル編集後')

    def test_update_diary_failure(self):
        response = self.client.post(reverse_lazy('diary_update', kwargs = {'pk':999}))
...
class TestDiaryDeleteView(LoggedInTestCase):
    def test_delete_diary_success(self):
        ...
        response = self.client.post(reverse_lazy('diary_delete', kwargs = {'pk':diary.pk}))
